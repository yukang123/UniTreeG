...
class TreeSearchDiffusion(GaussianDiffusion):
    def __init__(
        self,
        *,
        betas,
            ...
            with th.no_grad():
                ...
                if loss:
                    if isinstance(loss, list):
                        formatted_loss = [f'{l:.3f}' for l in loss]
                        indices.set_postfix({'loss': ', '.join(formatted_loss)}, refresh=False)
                    else:
                        indices.set_postfix({'loss': f'{loss:.3f}'}, refresh=False)
    ...
    def ddim_sample(
        ...
        if getattr(guidance_kwargs, "dps", False) and t[0] > self.t_end and use_guidance:
            dps_out = self.condition_mean_dps(cond_fn=cond_fn, mean_pred=mean_pred, x=x, t=t,
                                                model=self._wrap_model(model), model_kwargs=model_kwargs,guidance_kwargs=guidance_kwargs,                                        
                                                embed_model=embed_model, scale_factor=scale_factor)
            
            mean_pred = dps_out['cond_mean']
        ...
        if t[0] <= self.t_end:
            ...
            sample = mean_pred + sigma * torch.randn_like(x)


        elif self.search_type == 'gs_grad':
            gs_kwargs = vars(guidance_kwargs.gs) if guidance_kwargs.gs else None
            pred_xstart = out["pred_xstart"]

            grad_x0_out = self.grad_xstart(t=t, pred_xstart=pred_xstart, embed_model=embed_model,
                                           scale_factor=scale_factor, cond_fn=cond_fn,
                                           model_kwargs=model_kwargs, gs_kwargs=gs_kwargs)
            
            loss = grad_x0_out['loss']
            
            epsilon = 1e-10
            g_ascent = grad_x0_out['guided_x0'] - grad_x0_out['original_x0']
            ...
            d_sample = torch.randn_like(x) * sigma

            guidance_rate = gs_kwargs.get('guidance_rate', 1.)
        ...
        elif self.branch_size > 1 and self.search_type == 'TreeG-SD':
            gs_kwargs = vars(guidance_kwargs.gs) if guidance_kwargs.gs else None
        
            pred_xstart = out["pred_xstart"]
            dc_kwargs = getattr(guidance_kwargs, 'dc', None)

            guided_x0_out = self.guided_xstart_search(t=t, pred_xstart=pred_xstart, mean_pred=mean_pred,
                                                        embed_model=embed_model, scale_factor=scale_factor,
                                                    model_kwargs=model_kwargs, gs_kwargs=gs_kwargs)
            
            guided_x0 = guided_x0_out['guided_x0']
            loss = guided_x0_out['loss']
            mean_pred = guided_x0_out['mean_pred']
            
            
            if gs_kwargs['dsg']:
                epsilon = 1e-10
                g_ascent = guided_x0 - guided_x0_out['original_x0']
                g_ascent_norm = torch.linalg.norm(g_ascent.view(g_ascent.size(0), -1), dim=1)
                g_ascent_norm = g_ascent_norm.unsqueeze(1).unsqueeze(2).unsqueeze(3)
                ...
                ## TODO: check
                eps_new = self._predict_eps_from_xstart(x, t, guided_x0)

                x_next_mean = (
                        guided_x0 * th.sqrt(alpha_bar_prev)
                        + th.sqrt(1 - alpha_bar_prev - sigma ** 2) * eps_new
                )

                sample = x_next_mean + sigma * torch.randn_like(x)

           

        elif self.branch_size > 1 and self.search_type == 'TreeG-SC':
            xcurrent_search_out = self.xcurrent_search(model=self._wrap_model(model), t=t, mean_pred=mean_pred, g_coeff=sigma,
                                            embed_model=embed_model, scale_factor=scale_factor, 
                                            model_kwargs=model_kwargs,scg_kwargs=scg_kwargs)
            
            sample = xcurrent_search_out['x_next']
            loss = xcurrent_search_out['loss']
        ...
        return {"sample": sample, "pred_xstart": out["pred_xstart"], "loss": loss}

    def guided_xstart_search(
                        self,
                        t,
                        pred_xstart,
                        mean_pred,
        ...
        num_samples = self.branch_size
        batch = pred_xstart.shape[0] // self.active_size
        _, C, H, W = pred_xstart.shape

        n_iter = gs_kwargs.get("n_iter", 1)
        ...
        for _ in range(n_iter):
            ...
            alpha_bar_t = _extract_into_tensor(self.alphas_cumprod, t, sample.shape)
            scale_t = torch.sqrt(1 - alpha_bar_t)
            pred_xstart_scale_t = scale_t / torch.sqrt(1+scale_t**2)
            var_scale = gs_kwargs.get("pred_xstart_scale", 1.)
            ...
            if embed_model is not None:
                chunk_size = gs_kwargs.get("chunk_size", 64)
                if sample_.shape[0] > chunk_size:
                    sample_chunks = torch.chunk(sample_, sample_.shape[0]//chunk_size, dim=0)
                    sample_ = torch.cat([_decode(sample_chunk, embed_model, scale_factor=scale_factor) for sample_chunk in sample_chunks], dim=0)
                else:
                    sample_ = _decode(sample_, embed_model, scale_factor=scale_factor)


            total_log_prob = 0
            for rule_name, rule_target in model_kwargs["rule"].items():
                gen_rule = _extract_rule(rule_name, sample_)
                #y_ = rule_target.repeat(num_samples, 1)
                y_ = rule_target.unsqueeze(1).expand(-1,num_samples, -1).contiguous().view(-1, rule_target.shape[-1])
                log_prob = - LOSS_DICT[rule_name](gen_rule, y_)
                # if record:
                #     each_loss[rule_name] = -log_prob.view(batch, -1)
                total_log_prob += log_prob * gs_kwargs.get(rule_name, 1.)

            ## for each batch, select the top active_size samples
            total_log_prob = total_log_prob.view(batch, -1)
            top_log_prob, top_ind = torch.topk(total_log_prob, self.active_size, dim=1, largest=True)
            ...
            # pred_xstart = pred_xstart.view(-1, *pred_xstart.shape[2:])

            loss.append(- top_log_prob.mean().item())

                

        return {'guided_x0': pred_xstart_new, 'original_x0':pred_xstart, 'mean_pred':mean_pred, 'loss': loss}
    ...
    def xcurrent_search(
        ...
        num_samples = self.branch_size
        batch = mean_pred.shape[0] // self.active_size
        _, C, H, W = mean_pred.shape

        sample = torch.repeat_interleave(mean_pred, repeats=num_samples, dim=0)
        g_coeff = torch.repeat_interleave(g_coeff, repeats=num_samples, dim=0)
        sample = sample + g_coeff * torch.randn_like(sample)
        ...
        # pred_xstart = self._predict_xstart_from_eps(sample, t, eps)
        c_size = 64
        chunk_size = scg_kwargs.get("chunk_size", c_size) if scg_kwargs else c_size
        t = t.repeat(num_samples)
        cloned_model_kwargs = {"y": model_kwargs["y"].repeat(num_samples)}

        # Chunking the sample and t if necessary
        if sample.shape[0] > chunk_size:
            sample_chunks = torch.chunk(sample, sample.shape[0] // chunk_size, dim=0)
            t_chunks = torch.chunk(t, t.shape[0] // chunk_size, dim=0)
            eps_chunks = [
                model(sample_chunk, self._scale_timesteps(t_chunk), **{k: v[t_chunk] for k, v in cloned_model_kwargs.items()})
                for sample_chunk, t_chunk in zip(sample_chunks, t_chunks)
            ]
        ...
        if embed_model is not None:
            chunk_size = scg_kwargs.get("chunk_size", 64) if scg_kwargs else 64
            if pred_xstart.shape[0] > chunk_size:
                pred_xstart_chunks = torch.chunk(pred_xstart, pred_xstart.shape[0]//chunk_size, dim=0)
                pred_xstart = torch.cat([_decode(pred_xstart_chunk, embed_model, scale_factor=scale_factor) for pred_xstart_chunk in pred_xstart_chunks], dim=0)
            else:
                pred_xstart = _decode(pred_xstart, embed_model, scale_factor=scale_factor)
        ...
        for rule_name, rule_target in model_kwargs["rule"].items():
            gen_rule = _extract_rule(rule_name, pred_xstart)
            y_ = torch.repeat_interleave(rule_target, repeats=num_samples, dim=0)
            log_prob = - LOSS_DICT[rule_name](gen_rule, y_)
            total_log_prob += log_prob * scg_kwargs.get(rule_name, 1.)
        ...
        total_log_prob = total_log_prob.view(batch, -1)
        if softmax:
            # Convert log probabilities to probabilities via softmax with temperature scaling
            prob = torch.softmax(total_log_prob / temperature, dim=1)

            # Sample indices from the probability distribution for each batch.
            top_ind = torch.multinomial(prob, self.active_size, replacement=False)
            top_log_prob = total_log_prob.gather(1, top_ind)
        else:
            top_log_prob, top_ind = torch.topk(total_log_prob, self.active_size, dim=1, largest=True)

        sample = sample.contiguous().view(batch, -1, C, H, W)
        sample = sample[torch.arange(batch).unsqueeze(-1), top_ind]
        sample = sample.contiguous().view(-1, C, H, W)

        return {'x_next': sample, 'loss': - top_log_prob.mean().item()}
    ...
    def grad_xstart(
        ...
        pred_xstart_new = pred_xstart
        n_iter = gs_kwargs.get("n_iter", 1)

        with torch.enable_grad():
            for _ in range(n_iter):
                x0 = pred_xstart_new.detach().requires_grad_(True)
                
                if embed_model is not None:
                    chunk_size = gs_kwargs.get("chunk_size", 128)
                    if x0.shape[0] > chunk_size:
                        sample_chunks = torch.chunk(x0, x0[0]//chunk_size, dim=0)
                        x0 = torch.cat([_decode(sample_chunk, embed_model, scale_factor=scale_factor) for sample_chunk in sample_chunks], dim=0)
                    else:
                        x0 = _decode(x0, embed_model, scale_factor=scale_factor)


                log_probs = cond_fn(x0, self._scale_timesteps(t), **model_kwargs)
                gradient = th.autograd.grad(log_probs.sum(), x0)[0]

                pred_xstart_new = x0 + gs_kwargs.get("pred_xstart_scale", 1.) * gradient


        return {'guided_x0': pred_xstart_new, 'original_x0':pred_xstart, 'loss': - log_probs.mean().item()}
        


    def condition_mean_dps(
            self,
            cond_fn,
            mean_pred,
            x,
            ...
            step_size = getattr(guidance_kwargs, "dps", 1.)


            xt = x.detach().clone().requires_grad_(True)

            chunk_size = 64                      # raise if you have more head‑room
            grad_buffer = torch.zeros_like(xt)   # where we accumulate ∂log p/∂xt
            log_probs_list = []                  # keep numerical values only (optional)

            with torch.enable_grad():
                for i in range(0, xt.shape[0], chunk_size):
                    end = min(i + chunk_size, xt.shape[0])     # handle the last, shorter slice
                    curr_bs = end - i
                    # ─────────────────── slice the micro‑batch ────────────────────────────
                    chunk   = xt[i : end]            # view, not copy
                    t_chunk = t[i : end]
                    mkw     = {
                        "y":    model_kwargs["y"][i : end],
                        "rule": {k: v[i : end] for k, v in model_kwargs["rule"].items()},
                    }

                    try:
                        # ---- forward through the diffusion model -------------------------
                        eps = model(chunk, self._scale_timesteps(t_chunk), **mkw)

                        # ---- forward through the constraint function ---------------------
                        pred_xstart = self._predict_xstart_from_eps(chunk, t_chunk, eps)
                        log_p       = cond_fn(pred_xstart, self._scale_timesteps(t_chunk), **mkw)

                        # ---- backward: get ∂log p/∂chunk only, then free graph -----------
                        grad_chunk = th.autograd.grad(
                            log_p.sum(),
                            chunk,
                            retain_graph=False,
                            create_graph=False,
                        ...
                        print(f"OOM at chunk {i}–{i+chunk_size}: {e}")
                        th.cuda.empty_cache()
                        import gc; gc.collect()
                        raise e

                    # ───────────────── accumulate & clean up ──────────────────────────────
                    grad_buffer[i : end] = grad_chunk.detach()          # save grad
                    log_probs_list.append(log_p.detach())                    # values only

                    del eps, pred_xstart, log_p, grad_chunk
                    th.cuda.empty_cache()

            # ─────────── final tensors (no graphs attached) ───────────────────────────────
            gradient  = grad_buffer                      # ∂(Σ log p)/∂xt
            log_probs = th.cat(log_probs_list, dim=0)    # optional



            gradient = gradient / th.sqrt(-log_probs.view(x.shape[0], 1, 1, 1) + 1e-12)

            cond_mean = mean_pred + step_size * gradient.float()

            # print("log_probs shape:", log_probs.shape)

            return {'cond_mean': cond_mean, 'log_probs': log_probs}
